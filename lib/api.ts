...
const ADMIN_PROJECTS_ENDPOINT = "http://127.0.0.1:8000/admin/projects";
...
export async function fetchAllProjects(): Promise<BackendProject[]> {
  const response = await fetch(ADMIN_PROJECTS_ENDPOINT, {
    cache: "no-store",
  });
...
export function mapBackendStatusToStage(status: string): StaffingStage {
  ...
  if (normalizedStatus.includes("final") || normalizedStatus.includes("confirm")) {
    return "Awaiting Final Confirmation";
  }

  if (normalizedStatus.includes("confirmed") || normalizedStatus.includes("complete")) {
